 ...
 * @returns {undefined}
 */
var newPlot2DArea = function(input) {

    var canvas = document.getElementById(input.canvas_id);
    var ctx;
 ...
 * @returns {unresolved}
 */
var newColorBar = function(input) {

    var imgData;
    var ctx;

    var colorbar = document.getElementById(input.canvas_id);

    colorbar.width = $(colorbar).css("width").match(/(\d+)px/)[1];
    colorbar.height = $(colorbar).css("height").match(/(\d+)px/)[1];

    if (colorbar.getContext !== null) {
        ctx = colorbar.getContext('2d');

        imgData = ctx.createImageData(colorbar.width, colorbar.height);

        var x = 0;
        var k = 0;
        for (var j = 0; j < colorbar.height; j++)
        {
            for (var i = 0; i < colorbar.width; i++)
            {

                x = input.colormap.min + (input.colormap.max - input.colormap.min) * (colorbar.height - 1 - j) / (colorbar.height - 1);
...
{
    var slope = 1 / (input.max - input.min);
    var intercept = -slope * input.min;

    var output = {};

    Object.defineProperty(output, 'max', {get: function() {
            return input.max;
        }});

    Object.defineProperty(output, 'min', {get: function() {
            return input.min;
        }});
...
function CanvasViewport(input) {
    ...
    output.addLayer = function(layer) {
        output.layers[output.layers.length] = layer;
    };
    ...
    output.element.mousedown(function(e) {
        mouse.down = true;
        mouse.downx = e.pageX - output.element.offset().left;
        mouse.downy = e.pageY - output.element.offset().top;
